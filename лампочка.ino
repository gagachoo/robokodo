// Этот блок команд выполняется один раз для инициализации на устройстве
void setup() {
  pinMode(13, OUTPUT); // Светодиод светится ярко
  pinMode(12, OUTPUT); //  
  Serial.begin(9600); // Простушиваем порт на скорости 9600 кбс
}

// Этот блок команд выполняется постоянно (цикл)
void loop() { 
  digitalWrite(13, HIGH); // Включение синего светодиода
    Serial.println("Включился синий светодиод");
  digitalWrite(12, LOW);  // Выключение красного светодиода
    Serial.println("Выключился крысный светодиод");
  delay(1000);            // Задержка 1 сек.
  digitalWrite(13, LOW);  // Выключение синего светодиода
    Serial.println("Выключился синего светодиод");
  digitalWrite(12, HIGH); // Включение красного светодиода
    Serial.println("Включился красный светодиод");
  delay(1000);            // Задержка 1 сек.

}